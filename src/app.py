...
class EmotionDetectionApp:
    def __init__(self):
        ...
        if not CV2_AVAILABLE:
            st.warning("⚠️ OpenCV not available. Webcam features will be disabled.")
            self.detector = None
            self.emotion_stats = {}
    ...
    def render_webcam_mode(self):
        ...
        if not CV2_AVAILABLE:
            st.error("""
            ❌ Webcam functionality is not available in this environment.
            
            **Possible reasons:**
            - OpenCV is not properly installed
            - Camera access is not supported
            - Running in a cloud environment without camera access
            
            Please use **Image Upload** mode instead.
            """)
            return
        
        # Check if detector is available
        if not self.detector:
            st.error("❌ Emotion detector is not initialized. Please check the model files.")
            return
    ...
    def run(self):
        ...
        # Navigation - adjust based on OpenCV availability
        st.sidebar.markdown("---")
        st.sidebar.subheader("🎮 Navigation")
        
        if CV2_AVAILABLE and self.detector:
            app_modes = ["🏠 Home", "👤 My Profile", "📷 Live Webcam", "🖼️ Image Upload", "🎨 Visualizations"]
        else:
            app_modes = ["🏠 Home", "👤 My Profile", "🖼️ Image Upload", "🎨 Visualizations"]
            st.sidebar.warning("⚠️ Webcam mode disabled")
        
        app_mode = st.sidebar.selectbox("Choose Mode", app_modes, index=0)
